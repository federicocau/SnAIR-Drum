//~ Copyright (C) 2014-2016: Willem Vree, contributions St�phane David.
...
function Wijzer (xss, ymins, ymaxs, times, tixlb, lbtix, tixbts) {  // create the music cursor
    ...
    // posizione y metronomo 'attaccato' alla stanghetta
    ...
    this.hmargin = 200;     // horizontal scroll margin
        ...
        for (ts = ts_p; ts; ts = ts.ts_next) {
            ...
            for (g = ts.extra; g; g = g.next) {
                if (g.type == TEMPO && g.tempo_notes) {
                    dtmp = g.tempo_notes.reduce (function (sum, x) { return sum + x; });
...
function lineChk () {
    ...
    $('#notation').css ('text-align', opt.ctrnot ? 'center' : 'center');
}
...
function checkMenu (evt) {
    ...
    // chiamo due volte msc_resize() per avere uno scale completo
    case 'autscl': msc_resize (); msc_resize (); break;
...
// new code
  ...
  var longThrow = -0.1;
    ...
    // code strumenti -> ottimizzare
   /* var queueS = Queue();
    var queueK = Queue(); 
    var queueH = Queue();
    var queueTom1 = Queue();
    var queueTom3 = Queue();
    var queueCrash = Queue();*/
    
    // code per gli strumenti
    var codaStrum = [];
    for(n=0; n<10; n++)
        codaStrum[n] = Queue();
        
    
    // mappatura suoni - nomi - code - tasti (solo tramite tastiera)
    /*
    var suoni = [
        { key: "D2", name: "snare", coda: queueS, button: [67, 86]},
        { key: "B1", name: "kick", coda: queueK, button: [70, 71]},
        { key: "F#2", name: "hihat", coda: queueH, button: [65]},
        { key: "C3", name: "hitom", coda: queueTom1, button: [83] },
        { key: "A2", name: "floortom", coda: queueTom3, button: [88] },
        { key: "C#3", name: "crash", coda: queueCrash, button: [90] }
    ];*/
    ...
    // mappatura suoni/figure
    var suoni = [
            { name: 'hitom', key: 'C3', dimension: [0.08, 0.07, 1, 1], position: [-0.085, 0.15, 0.020], rotation: [180.8, 0, 0], coda: codaStrum[0] },         
            { name: 'snare', key: 'D2', dimension: [0.1, 0.07, 1, 1], position: [-0.10, 0.123, 0.09], rotation: [180.8, 0, 0], coda: codaStrum[1] },
            { name: 'hat', key: 'F#2', dimension: [0.1, 0.07, 1, 1], position: [-0.23, 0.139, 0.07], rotation: [180.8, 0, 0], coda: codaStrum[2]},  
            { name: 'crash', key: 'C#3', dimension: [0.13, 0.07, 1, 1], position: [-0.2, 0.21, -0.01], rotation: [180.8, 0, 0], coda: codaStrum[3] },
            { name: 'crash2', key: 'C#3', dimension: [0.13, 0.075, 1, 1], position: [0.045, 0.22, -0.01], rotation: [180.8, 0, 0], coda: codaStrum[4] },
            { name: 'floortom', key: 'A2', dimension: [0.1, 0.05, 1, 1], position: [0.09, 0.111, 0.13], rotation: [180.8, 0, 0], coda: codaStrum[5] },
            { name: 'midtom', key: 'B2', dimension: [0.08, 0.07, 1 ,1], position: [0.001, 0.15, 0.020], rotation: [180.8, 0, 0], coda: codaStrum[6] },
            { name: 'splash', key: 'G3', dimension: [0.06, 0.04, 1 ,1], position: [-0.03, 0.181, 0.003], rotation: [180.8, 0, 0], coda: codaStrum[7] }, 
            { name: 'ride', key: 'D#3', dimension: [0.13, 0.07, 1, 1], position: [0.1, 0.132, 0.06], rotation: [180.8, 0, 0], coda: codaStrum[8] },       
            { name: 'china', key: 'E3', dimension: [0.1, 0.08, 1, 1], position: [0.18, 0.19, 0.14], rotation: [180.8, 74, 0], coda: codaStrum[9] }
        ...
        // se la coda non è vuota elimino e poi inserisco, altrimenti inserisco
        function pushCoda(indice, item) {
            if (!suoni[indice].coda.isEmpty()) {
                suoni[indice].coda.delete();
                suoni[indice].coda.insert(item);
            } else
                suoni[indice].coda.insert(item);
        }
        
        // genero ogni figura
        for(j=0; j<suoni.length; j++){
            // come evitare questo abominio?
            var planeGeometry = new THREE.PlaneGeometry(suoni[j].dimension[0], suoni[j].dimension[1], suoni[j].dimension[2], suoni[j].dimension[3]);
            var planeMaterial = new THREE.MeshLambertMaterial({wireframe: false, color: 0xff0000, transparent: true, opacity: 0.0});
            var plane = new THREE.Mesh(planeGeometry, planeMaterial);
            plane.position.set(suoni[j].position[0], suoni[j].position[1], suoni[j].position[2]); // posizione
            plane.rotation.set(suoni[j].rotation[0], suoni[j].rotation[1], suoni[j].rotation[2]); // rotazione
            plane.name = suoni[j].name;
             
            scene.add(plane); 
            //meshes[i] = plane;
            
            // rendo la figura interattiva
            interactiveButtons[j] = new PushButton(
                new InteractablePlane(plane, Leap.loopController),
                {
                  ...
                  longThrow: 0
                    ...
                    if(elmed) // se il brano è stato caricato
                     ...
                     for(k=0; k<suoni.length; k++){
                         if(suoni[k].name === mesh.name){
                            if(elmed){ 
                                var item = {name: suoni[k].key, time: elmed.currentTime.toFixed(3)};
                                // lo pusho nella coda
                                pushCoda(k, item); 
                                playSound(suoni[k].name);
                                break trovaSuono;
                            }
                         }
                     }
    ...
    document.getElementById("fknp").addEventListener("change", function() {
        ...
        document.getElementById('aud').addEventListener('play', function () {
            ...
            document.onkeydown = function (e) {
                ...
                setButton:
                for (k = 0; k < suoni.length; k++) {
                    // scorro il vettore dei tasti 
                    for (j = 0; j < suoni[k].button.length; j++) {
                        // se la key premuta è uguale ad uno dei valori trovati nel vettore (che corrisponde ai codici)
                        if (key === suoni[k].button[j]) {
                            // creo l'item 
                            var item = {name: suoni[k].key, time: elmed.currentTime.toFixed(3)};
                            // lo pusho nella coda
                            pushCoda(k, item);
                            break setButton; // se lo trovo esco dal ciclo for più esterno
                        }
                    }
                }
            };*/
            ...
            controller();
            function controller() {           
                //console.log(sheet.notes.length);
                ...
                if(i <= sheet.notes.length-1){ // nuovo if
                    ...
                    if (nStrumenti > 1) {
                        ...
                        Array.prototype.contains = function (obj) {
                            var i = this.length;
                            while (i--) {
                                //console.log("coda: " + this[i].name + "; suoni: " + obj.name);
                        ...
                        var info = 0;
                        // controllo a coppie e quindi avro': (num. strumenti) - 1
                        for (n = 0; n < nStrumenti - 1; n++)
                            // se la coppia di code non è vuota
                            if (!(code[n].queue).isEmpty() && !(code[n + 1].queue).isEmpty())
                                // incremento la coppia di code
                                info++;
                        ...
                        // incremento i del numero degli strumenti diversi suonati
                        i += nStrumenti;
                        code = [];    // resetto la coda strumenti corretta per ogni nota suonata
                        codeErr = []; // resetto la coda strumenti errata per ogni nota suonata					
                    }
                    ...
                    // altrimenti controllo di quale strumento si tratta (controllo nell'obj dei suoni)
                        ...
                        if (q) {
                            // se la coda del suono esatto non è vuota
                            if (!(q.coda).isEmpty()) {
                                // estrapolo l'item
