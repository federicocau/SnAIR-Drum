...
//        };
    ...
    var rightTime = 0; // tempo esatto in cui suonare
        ...
        function pushCoda(indice, item) {
            if (!figures[indice].coda.isEmpty()) {
                figures[indice].coda.delete();
            ...
            function controller() {           
                ...
                if(i <= sheet.notes.length-1){ // nuovo if
                    ...
                    // altrimenti controllo di quale strumento si tratta (controllo nell'obj dei suoni)
                        ...
                        var q;
                        searchInstrument:
                                for (s = 0; s < figures.length; s++) {
                            if (sheet.notes[i].name === figures[s].key) {
                                q = {key: figures[s].key, name: figures[s].name, coda: figures[s].coda};
                                ...
                                if ((itemS.time >= deltaSx) && (itemS.time <= deltaDx)) {
                                    ...
                                    checkErrors:
                                        for (k = 0; k < figures.length; k++) {
                                            ...
                                            if (figures[k].key !== q.key) {
                                                // controllo che la sua coda non sia vuota
                                                if (!figures[k].coda.isEmpty()) {
                                                    var item = figures[k].coda.shift();
                                                    figures[k].coda.delete();
